...
// Derive a 1-2 letter project key from the project's identifier (slug or
// numeric id). Mirrors the rule in `mapProject` so we can compute keys on
// the fly from a WP's `_links.project.href` without a separate /projects
// fetch.
function projectKeyFromIdentifier(identifier) {
  if (!identifier) return "WP";
  const letters = identifier.match(/[A-Za-z]/g);
  const head = (letters || [identifier[0]]).slice(0, 2).join("").toUpperCase();
  return head || "WP";
}

// projectIndex: identifier (string) → projectKey (e.g. "WA")
// When no map is supplied, derive the key from the WP's project href so
// callers (e.g. routes returning a single WP, or large list fetchers that
// don't want the extra /projects round-trip) get a meaningful key without
// preloading the whole project index.
export function mapWorkPackage(wp, opts = {}) {
  const { projectKeyByHref = {} } = opts;
  const projectHref = wp._links?.project?.href;
  const projectIdentifier = projectHref ? projectHref.split("/").pop() : "";
  const pKey =
    projectKeyByHref[projectHref] || projectKeyFromIdentifier(projectIdentifier);
