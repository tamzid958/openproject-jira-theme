...
// `statusColor`, `typeColor`, `priorityColor`, `priorityPosition`. Pass them
// when you want every consumer to read API truth without doing its own
// lookup; omit them to fall through with `null` for those fields.
export function mapWorkPackage(wp, opts = {}) {
  const { statuses, types, priorities } = opts;
  const findById = (list, id) => {
    if (id == null || !Array.isArray(list)) return null;
    for (const item of list) if (String(item?.id) === String(id)) return item;
    return null;
  };
  ...
  const statusRecord = findById(statuses, statusId);
  const typeRecord = findById(types, typeId);
  const priorityRecord = findById(priorities, priorityId);
  return {
    id: `wp-${wp.id}`,
    nativeId: wp.id,
