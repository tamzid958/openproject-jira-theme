...
export function mapVersionToSprint(v) {
  ...
  // OpenProject versions only have open/locked/closed; we infer "planned"
  // for an open version that has no start date (or a start date still in
  // the future) so the UI can offer "Start sprint" before kickoff.
  const isOpen = (v.status || "open") === "open";
  let state;
  if (!isOpen) {
    state = "closed";
...
const STORY_POINTS_FIELD = process.env.OPENPROJECT_STORY_POINTS_FIELD || "storyPoints";
...
function pickStoryPoints(wp) {
  ...
  if (wp.estimatedTime) {
    const m = String(wp.estimatedTime).match(/PT(\d+)H/);
...
// list lives on the individual /api/v3/roles/{id} resource. We only model
// User principals here — Group memberships expand to their member users
// server-side, so per-user effective permissions already include them.

export function mapMembership(m) {
  const principalHref = m._links?.principal?.href || "";
  // Skip non-User principals (groups, placeholders) — caller filters.
  const isUser = /\/api\/v3\/users\//.test(principalHref);
  const rolesEmbedded = m._embedded?.roles || [];
  const rolesLinked = m._links?.roles || [];
  return {
    id: String(m.id),
    isUser,
    principalId: idFromHref(principalHref),
    projectId: idFromHref(m._links?.project?.href),
    projectHref: m._links?.project?.href || null,
    roleIds: (rolesEmbedded.length ? rolesEmbedded : rolesLinked)
      .map((r) => idFromHref(r.href || r._links?.self?.href))
      .filter(Boolean),
    roleNames: (rolesEmbedded.length ? rolesEmbedded : rolesLinked)
      .map((r) => r.name || linkTitle(r))
      .filter(Boolean),
  };
}
