...
async function runAction(actionFn, input, fallback) {
  try {
    const res = await actionFn(input);
    ...
    // Network failure to the action endpoint (offline, server unreachable).
    // Tag it as a NETWORK_FAIL so the offline queue can decide whether to
