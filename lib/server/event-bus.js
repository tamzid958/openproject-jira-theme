...
// Soft cap — if a future bug ever leaks subscribers (route rotation
// without `cancel`, etc.) we want to notice in logs instead of letting
// the Set grow forever.
...
export function subscribe(filter, handler) {
  ...
  if (subscribers.size > MAX_SUBSCRIBERS) {
    console.warn(`[event-bus] subscriber count exceeded ${MAX_SUBSCRIBERS} (now ${subscribers.size}); possible leak`);
  }
